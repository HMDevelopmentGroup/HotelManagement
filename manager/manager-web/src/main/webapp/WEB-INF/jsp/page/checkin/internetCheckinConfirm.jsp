...
<script>
    layui.use(['table','jquery'],function() {
        ...
        table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'edit'){
                $.ajax({
                    url:"${pageContext.request.contextPath}/internetOrderCompileStatus/"+data.ioid,
                    success:function (data) {
                        if(data==1){
                            layer.confirm("入住成功",{btn:"确定"},function () {
                                location.reload();
                            });
                        }
                    }
...
<script type="text/html" id="compile">
    <a class="layui-btn layui-btn-xs" lay-event="edit">客人已入住</a>
