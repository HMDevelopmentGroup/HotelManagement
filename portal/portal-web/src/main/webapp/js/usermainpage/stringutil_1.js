...
function ChangeCodeImg(imgID) {
    ...
    var mainDomainUrl = $("#hUseingDomain").val() || $("#hParentDomain").val();
    if (mainDomainUrl != undefined && mainDomainUrl != null && mainDomainUrl != "") {
        if (mainDomainUrl == "www.jinjiang.com") {
            mainDomainUrl = "www.jinjianginns.com";
        }
        if (mainDomainUrl.indexOf("http://") < 0) {
            mainDomainUrl = "http://" + mainDomainUrl;
        }
        a.src = mainDomainUrl + "/Handlers/CodeImage.aspx?TypePara=login&num=" + num;
    } else {
        a.src = "http://www.jinjianginns.com/Handlers/CodeImage.aspx?TypePara=login&num=" + num;
    }
    $("#randomNum").val(num);
}
...
+    "<p>系统检测到您的账户可能存在安全风险，为保护您的会员权益，请先修改登录密码，再进行后续操作，谢谢！</p>"
+    "<div class='update'>"
...
function CheckPromotionRateCode(rateCode) {
    var hasFlg = false;
    if (promontionRateCodeData != undefined && promontionRateCodeData != null) {
